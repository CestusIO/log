...
# Check for Windows or Linux/Unix
ifeq ($(OS),Windows_NT)
    msg = @powershell -Command "Write-Host \"$(1)\" -ForegroundColor Cyan"
else
...
ifeq ($(OS),Windows_NT)
    b_command = export GOOS=$(GOOS); export GOARCH=$(GOARCH); export CGO_ENABLED=0; go build -ldflags "$(LDFLAGS) -X code.cestus.io/libs/buildinfo.name=$(APP)" -o $(BINDIR)/$(APP)-$(GOOS)-$(GOARCH)$(BINARY_$(GOOS)_ENDING) ./cmd/$(APP)/ ;
else
...
ifeq ($(OS),Windows_NT)
    be_command = export GOOS=$(GOOS); export GOARCH=$(GOARCH); export CGO_ENABLED=0; go build ./... ;
else
    be_command = export GOOS=$(GOOS); export GOARCH=$(GOARCH); export CGO_ENABLED=1; go build ./... ;
endif
...
# for each iterations use build message
fbe_platforms =$(foreach GOOS, $(PLATFORMS),$(foreach GOARCH, $(ARCHITECTURES),$(b_platform)))
...
# foreach iterations to do multi platform build
fbe_command = $(foreach GOOS, $(PLATFORMS),\
	$(foreach GOARCH, $(ARCHITECTURES),$(be_command)))
...
ifeq ($(OS),Windows_NT)
		...
		echo "ginkgo $${dir}/..."; \
		(cd "$${dir}" && \
		export CGO_ENABLED=0; ginkgo --output-dir=. ./...); \
		done; 
else
		set -e; for dir in $(ALL_GO_MOD_DIRS); do \
		echo "ginkgo $${dir}/..."; \
		(cd "$${dir}" && \
		export CGO_ENABLED=1; ginkgo -race --output-dir=. ./...); \
		done;
endif
